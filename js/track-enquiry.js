...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  async function claim(reference) {
    if (!window.KridiyaAuth) throw new Error("SERVICE_UNAVAILABLE");
  ...
  form.addEventListener("submit", async function (event) {
    ...
    if (window.KridiyaMotion && KridiyaMotion.label) KridiyaMotion.label(button, "Checking...", true);
    ...
    try {
      ...
      if (window.KridiyaMotion && KridiyaMotion.label) KridiyaMotion.label(button, button.dataset.label || "Verify and track", false);
      else button.textContent = button.dataset.label || "Verify and track";
    }
  });

  if (!window.KridiyaAuth) {
    message(errorBox, "Enquiry tracking is temporarily unavailable. Please try again in a moment.");
    return;
  }
  try {
    const user = await KridiyaAuth.currentUser();
    if (user && savedRef) await claim(savedRef);
  } catch (error) {
    message(errorBox, "We could not connect to enquiry tracking. Please try again in a moment.");
  }
});
